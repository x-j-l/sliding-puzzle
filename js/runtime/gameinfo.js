...
const IMG_ABOUT_SRC = 'images/about.png'
const IMG_ABOUT_WIDTH = 300
const IMG_ABOUT_HEIGHT = 200

const IMG_ABOUT_CONTENT_SRC = 'images/puzzle-about.png'
const IMG_ABOUT_CONTENT_WIDTH = '640'
const IMG_ABOUT_CONTENT_HEIGHT = '907'
...
export default class GameInfo {
  constructor() {
    ...
    // 关于按钮
    let aboutRatio = (databus.screenWidth * 0.12) / IMG_ABOUT_WIDTH
    this.btnAbout = new Button(
      IMG_ABOUT_SRC,
      this.btnHelp.x - (aboutRatio * IMG_ABOUT_WIDTH + 10),
      databus.contentPaddingTop - (aboutRatio * IMG_ABOUT_HEIGHT + helpButtonPadding),
      aboutRatio * IMG_ABOUT_WIDTH,
      aboutRatio * IMG_ABOUT_HEIGHT
    )

    // 关于内容
    let aboutContentHeight = (databus.screenWidth / IMG_ABOUT_CONTENT_WIDTH) * IMG_ABOUT_CONTENT_HEIGHT
    this.aboutContent = new Button(
      IMG_ABOUT_CONTENT_SRC,
      0,
      databus.screenHeight - aboutContentHeight,
      databus.screenWidth,
      aboutContentHeight
    )
  ...
  tapGamePlaying(event) {
    ...
    if (databus.showAbout && this.aboutContent.isTapped(event.x, event.y)) {
      return databus.showAbout = false
    }
    ...
    if (this.btnAbout.isTapped(event.x, event.y)) {
      return databus.showAbout = true
    }
  ...
  renderGamePlaying(ctx) {
    ...
    if (databus.showAbout) {
      ctx.fillStyle = "black";
      ctx.globalAlpha = 0.6;
      ctx.fillRect(0, 0, databus.screenWidth, databus.screenHeight);
      ctx.globalAlpha = 1;
      this.aboutContent.render(ctx)
    }
    if (databus.showHint) {
      ctx.drawImage(
        this.puzzleImg,
